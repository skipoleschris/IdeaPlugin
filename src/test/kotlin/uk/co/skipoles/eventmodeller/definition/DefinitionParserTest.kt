...
import uk.co.skipoles.eventmodeller.visualisation.SvgDocumentGenerator
...
fun main() {
  ...
  definition.getOrNull()?.entries?.forEach(::println)

  val visualisationModel = VisualisationModelGenerator.generate(definition.getOrNull()!!)
  println()
  visualisationModel.postIts.forEach(::println)
  println()
  visualisationModel.links.entries.forEach(::println)

  val image = SvgDocumentGenerator(visualisationModel).renderDocument().asPNG()
